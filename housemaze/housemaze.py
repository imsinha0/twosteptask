...
class HouseMaze:
  ...
  def reset(self, rng: jax.Array, params: EnvParams) -> Tuple[Observation, EnvState]:
    """
    ...
    return TimeStep(
          ...
          step_type=jnp.full((), StepType.FIRST, dtype=StepType.FIRST.dtype),
        )

  def step(
    self, rng: jax.Array, timestep: TimeStep, action: jax.Array, params: EnvParams
  ) -> Tuple[Observation, EnvState, jax.Array, jax.Array, dict]:
    if self.action_spec == "keyboard":
    ...
    return TimeStep(
        state=state,
        observation=observation,
        discount= (1 - terminated).astype(jnp.float32),
        reward=reward,
        step_type=jax.lax.cond(done, lambda: StepType.LAST, lambda: StepType.MID),
      )
